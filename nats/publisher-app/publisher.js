...
const logPrefix = `NATS Publisher (${process.pid}):\t`;
...
app.post('/publish', (req, res) => {
  ...
  try {
    nats.publish(subject, message, 'test-reply', err => {
      if (err) {
        log('message published with error');
        return res.status(500).send(err.message);
      }
      // guard condition neccessary due to a bug in nats: https://github.com/nats-io/nats.js/pull/286/files#r310898869
      if (!res.headersSent) {
